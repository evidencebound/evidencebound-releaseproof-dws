...
app = FastAPI(title="ReleaseProof Judge Surface", version="0.3.0")
...
<section class="hero"><div class="panel"><div class="kicker">Nutrient DWS Challenge · Judge Surface</div><h1>ReleaseProof</h1><p class="lead">A proof-carrying release manifest for document-driven autonomous work. Human review survives only when the exact source-grounded finding evidence still reproduces.</p><div class="badges"><span class="badge">HOSTED NUTRIENT DWS · PASS</span><span class="badge">PUBLIC CI · PASS</span><span class="badge warn">HOSTED DIFFERENTIAL · QUOTA BLOCKED</span></div><div class="row"><button onclick="runControlled()">Run controlled mechanism</button><button class="alt" onclick="loadLive()">Show hosted DWS evidence</button></div></div>
<div class="panel"><div class="kicker">Evidence summary</div><div class="metrics"><div class="metric"><div class="num">3</div><div class="small">PDFs processed by hosted DWS</div></div><div class="metric"><div class="num">27</div><div class="small">deterministic tests passing</div></div><div class="metric"><div class="num">1/1</div><div class="small">controlled review preserved after non-material revision</div></div><div class="metric"><div class="num">0/1</div><div class="small">review preserved after reviewed evidence changed</div></div></div></div></section>
<section class="section grid3"><div class="panel"><div class="kicker">Live document operation</div><h2 class="state ok">PASS</h2><p>Hosted Processor <code>/build</code>, JSON key-value extraction, response receipt hashes, page/bbox/confidence grounding.</p></div><div class="panel"><div class="kicker">Live trust decision</div><h2 class="state warntext">REVIEW_REQUIRED</h2><p>Hosted extraction produced a cross-document Shipment ID disagreement. ReleaseProof surfaced it instead of silently releasing.</p></div><div class="panel"><div class="kicker">Current external blocker</div><h2 class="state warntext">HTTP 402</h2><p>The account quota was exhausted before the hosted non-material-revision rerun. No success claim is made for that experiment.</p></div></section>
<section class="section proof"><div class="panel"><h2>Differential Reverification</h2><div class="callout">A file hash changing is not enough reason to discard human work. ReleaseProof reprocesses the current packet and reuses review only if the finding binding—logical document, field, normalized value, page, bounds and confidence—still matches. The current packet always receives a new manifest.</div><p class="small">Controlled evidence: unchanged reviewed slice preserves 1/1 prior review; changed reviewed slice preserves 0/1. Hosted repeat-extraction experiment remains quota-blocked and is kept separate.</p></div><div class="panel"><h2>Canonical hosted proof</h2><p class="small">Run <b>32215337912</b> · 3 documents · source grounding PASS · manifest generated · DWS receipts retained. Live calls are disabled on this public surface so a judge cannot consume the exhausted account quota.</p><button class="alt" onclick="loadLive()">Load evidence receipt</button></div></section>
